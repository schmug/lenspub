...
- *The impact class rises to `major`.* The `trivial` class is defined over changes confined to `/interpretation/presentation`, and `minor` is a closed list of weight, source, and advisory-subscription changes ([Adaptation Model](adaptation-model.md), Section 4); a change under `/extensions` matches neither, so it lands in `major` — everything else — classified identically to an Adaptation Policy change, a `/privacy` edit, or a removal. The consequence is unconditional rather than a matter of degree: no `autoAcceptCeiling` reaches `major`, since the presets top out at `minor` under `explorer` and the ceiling admits no higher value (Sections 5 and 6). A view switch can therefore never be auto-accepted under any policy, preset or custom, and always requires explicit user review.
