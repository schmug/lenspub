...
`LensChangeProposal` has no JSON Schema in v0.1: proposals are Adaptation State, never exchanged between parties, and their serialization is engine-internal. Defining a normative schema for them is roadmap work (see [Roadmap](../docs/roadmap.md)). The diff fragment they carry, however, MUST already conform to the Lens Diff change-object structure, so that proposal previews, subscription reviews, and history diffs all read identically. A `proposal-preview` diff for the running-example lens `avery-daily`:
